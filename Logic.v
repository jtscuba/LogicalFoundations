...
Theorem not_both_true_and_false : forall P : Prop,
  ~ (P /\ ~P).
Proof.
  intros P [Hp HNp]. unfold not in HNp.
  apply HNp, Hp.
Qed.
(** [] *)
...
Proof.
  (* Introduce the type and prop args,
     along with the universal quantification. *)
  intros X P Hx.
  (* Introduce the witness and statement about the witness. *)
  intros [x0 E].
  (* Expand to make the not logic clearer *)
   unfold not in E. apply E.
   apply Hx.
Qed.
(** [] *)
  ...
  + induction l as [| x' l' IHl'].
    ...
    - simpl. intros [H | H].
      ...
      * exists x'. split. 
        ...
        { left. reflexivity. } 
      (* In y (map f l') *)
      * apply IHl' in H. destruct H as [x'' [Fx''y In_x_l'] ].
        exists x''. split. 
          { apply Fx''y. } 
          { right. apply In_x_l'. }
  (* backward direction *)
  + intros [x' [Fxy In_x'_l] ].
    (* Use result from above to make this easy. *)
    rewrite <- Fxy. apply In_map. apply In_x'_l.
Qed.
(** [] *)
...
Proof.
  split.
  (* Forward Direction *)
  + induction l as [| x l_tail IHl_tail].
    (* Base Case: l = [] *)
    - simpl. right. apply H.
    (* Inductive Step: l = x :: l_tail *)
    - simpl. intros [H | H].
      * left. left. apply H.
      * apply IHl_tail in H. apply or_assoc.
        right. apply H.
  (* Backward direction *)
  + intros [H | H].
    (* x in l*)
    - induction l as [ | h l_tail IHl_tail].
      (* base case never happens *)
      * inversion H.
      (* Inductive step l = h :: l_tail *)
      * simpl. simpl in H.
        (* split the disjuntive hypothesis *)
        destruct H as [Hha | Htail].
          (* h = a *)
          { left. apply Hha. }
          (* a in l_tail*)
          { right. apply IHl_tail, Htail. }
    (* x in l' *)
    - induction l as [| h l_tail IHl_tail].
      (* base case, l = [], l ++ l' = l' *)
      * simpl. apply H.
      (* l = *)
      * simpl. right. apply IHl_tail.
Qed.
...
Proof.
  ...
  (* Forward case *)
  + intros H.
  induction l as [| h t IHl_tail].
    - reflexivity.
    - simpl. split.
      * apply H. simpl. left. reflexivity.
      * apply IHl_tail. simpl in H.
        intros x Hin. apply H. right. apply Hin.
  (* Backwards case *)
  + intros H. induction l as [| h t IHl_tail].
    (* Base case: assumption is false *)
    - intros x. simpl. intros H2. inversion H2.
    (* Inductive step *)
    - simpl. intros x [Heq | Hin].
      (* h = x case*)
      * simpl in H. destruct H as [Hph Hall_tail].
        rewrite <- Heq. apply Hph.
      (*In x t case*)
      * simpl in H. destruct H as [Hph Hall_tail].
        apply IHl_tail. apply Hall_tail. apply Hin.
Qed.
(** [] *)
...
Lemma rev_append_l2_is_appended : forall (X:Type) (l1 l2 : list X),
  (rev_append l1 l2) = ((rev_append l1 []) ++ l2).
Proof. intros X l1. induction l1 as [| h2 l1' IHl1'].
  - reflexivity.
  - intros l2. simpl. rewrite IHl1'. rewrite (IHl1' [h2]).
...
Proof.
  (* use functional existentiality so we only need to prove the
     output of the two functions is equal.*)
  intros X.
  apply functional_extensionality.
  intros x.
  (* Use induction on the list, IH will probably help *)
  induction x as [| h x' IHx'].
  + (* Base case x = [] *)
    unfold tr_rev. reflexivity.
  + (* Inductive step, x = h :: x' *)
     simpl. unfold tr_rev. simpl. 
     rewrite rev_append_l2_is_appended. 
     fold (tr_rev x'). rewrite IHx'. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  - destruct (evenb n) eqn:H.
    ...
    + rewrite -> evenb_S. rewrite -> H. simpl.
      destruct IHn' as [k' Hk']. exists (S(k')).
