...
Definition filter_even_gt7 (l : list nat) : list nat :=
  filter evenb (filter (fun n => leb 7 n) l).
...
Proof.
  ...
  - reflexivity.
  - simpl. rewrite -> map_app_distr, IHl'. reflexivity. Qed.
...
Compute fold_map (fun n => n) [1; 2; 3].

Example test_fold_map1: fold_map (fun n: nat => n*n) [2] = [4].
Proof. reflexivity. Qed.
...
Proof. intros X Y f l. induction l as [| h t IHt].
  - reflexivity.
  - unfold fold_map. unfold fold_map in IHt.
...
Proof.
  intros X Y Z f x y. 
  unfold prod_uncurry. 
  unfold prod_curry. reflexivity. Qed.

Theorem curry_uncurry : forall (X Y Z : Type)
                        (f : (X * Y) -> Z) (p : X * Y),
  prod_uncurry (prod_curry f) p = f p.
Proof.
  intros X Y Z f (x, y). unfold prod_uncurry, prod_curry.
  reflexivity. Qed.
(** [] *)
...
(** Recall the definition of the [nth_error] function:
   ...
   first note that a lists are zero indexed. Thus a list of length n
   will have elements 0, ..., n-1. Thus nth_error will hit the empty
   list case while recursing. Thus None will be returned.
