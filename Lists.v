...
Fixpoint subset (s1:bag) (s2:bag) : bool :=
  ...
  | h1 :: t1 => match s2 with
                | nil => false
                | _ => if beq_nat (count h1 s2) 0
                       then false
                       else subset t1 (remove_one h1 s2)
                end
  end.
...
- Search rev. rewrite -> nil_app, test_rev2, app_nil_r. reflexivity.
...
Proof.
  ...
  - reflexivity.
  (* inductive step: s = h :: s' *)
  - simpl. destruct h.
  (* two cases, h = 0, h != zero *)
    (** 
      case 1, h = 0, so we hit the true branch of remove_one
      and get the count in terms of the count of the tail 
      We use the lemma from above, and are done
    **)
    + simpl. rewrite -> leb_n_Sn. reflexivity.
    (**
      case 2, h != 0, so we hit the false branch of remove_one
      and get the count in terms of the count of the tail.
      we apply the inductive hypothesis and are done.
    **)
...
(* FILL IN HERE *)
  No values, all the constructors take a value of type baz, which
  we have no way to make from anything else.
*)
(** [] *)
