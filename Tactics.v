...
(* FILL IN HERE *)
when a goal exactly matches an existing lemma, theorem, or hypothesis
and you want to rewrite the equation w/o introducting quantified
variables explicitly. This lets us use generic theorems more easily.

rewrite is useful for swapping out part of a goal using a known fact
in order to produce an equivalent statement.
*)
(** [] *)
...
Example inversion_ex6 : forall (X : Type)
  ...
  x = z.
Proof.
intros X x y z l j contra1. inversion contra1. Qed.
...
Proof.
  ...
  - intros n eq. simpl. destruct n as [| n'].
    + inversion eq.
    + apply IHt. simpl. inversion eq. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros X Y l.
  induction l as [| h t IHt].
  - unfold split. intros l1 l2 eq. inversion eq. reflexivity.
  - simpl. destruct (split t) as [t1 t2]. destruct h. intros l1 l2.
    intros eq. destruct l1, l2.
    + inversion eq.
    + inversion eq.
    + inversion eq.
    + simpl. inversion eq.
      rewrite -> IHt.
      * (* the conclusion of IHt*) reflexivity.
      * (* the premice of IHt *) rewrite H1, H3. reflexivity. 
Qed.
...
Theorem bool_fn_applied_thrice :
  ...
  f (f (f b)) = f b.
Proof.
  intros f b. destruct (b) eqn:Heqn1.
  - destruct (f true) eqn:Heqn2.
    + rewrite Heqn2, Heqn2. reflexivity.
    + destruct (f false) eqn: Heqn3.
      * apply Heqn2.
      * apply Heqn3.
  - destruct (f false) eqn:Heqn2.
    + destruct (f true) eqn:Heqn3.
      * apply Heqn3.
      * apply Heqn2.
    + rewrite Heqn2, Heqn2. reflexivity.
Qed.
...
(** Give an informal proof of this lemma that corresponds to your
   ...
   Case (n = O).
    Case (m = O) -> true = true.
    Case (m = O) -> false = false.\u00e5
   Case (n = S n')
    Case (m = O) -> false = false.
    Case (m = S m')
      we need to show beq S n' S m' = beq S m' S n'
      Applying the definition of beq we simplify this to beq n' m' = beq m' n'.
      This is precisley the inductive hypotheses.
...
Proof.
  ...
  rewrite eq1, eq2. rewrite <- beq_nat_refl. reflexivity. 
Qed.
(** [] *)
...
Proof.
  ...
  - simpl. destruct (test h) eqn:Heq.
    + intros lf eq. inversion eq. rewrite <- H0. apply Heq.
    + apply IHt.
Qed.
  (* We are trying to prove that the first element
     in a filtered list passes the test that was used 
     to filter that list*)
(** [] *)
